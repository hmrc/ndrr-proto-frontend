...
trait Common {
  ...
  def address(address: String) =       VoaSummaryListRow("registration.checkAnswers.contactDetails.table.address",Seq(address),Some(Link(Call(method = "GET",url = uk.gov.hmrc.ndrrprotofrontend.controllers.routes.AddressController.onPageLoad.url),messageKey = "voa.change.link", linkId = "address-Id")))
